<template>
    <div ref="timelineRoot" class="dense-timeline">
...
<script setup lang="ts">
    ...
    const filteredEvents = computed(() => {
        return props.events.filter((e) => Boolean(resolveEventDate(e)));
    });
    ...
    const visibleDateRange = computed(() => {
        const { min, max } = fullDateRange.value;
        if (zoom.value === 'all') return { min, max };
        const anchor = new Date(max);
        if (zoom.value === 'year') {
            anchor.setFullYear(anchor.getFullYear() - 1);
        } else {
            anchor.setFullYear(anchor.getFullYear() - 10);
        }
        return { min: Math.max(min, anchor.getTime()), max };
    });
    const visibleEvents = computed(() => {
        const { min, max } = visibleDateRange.value;
        return filteredEvents.value.filter((evt) => {
            const date = resolveEventDate(evt);
    ...
    function onEnter(evt: EventRecord, e: MouseEvent) {
        ...
        hoverEvt.value = evt;
        const rootRect = timelineRoot.value?.getBoundingClientRect();
        if (rootRect) {
            tooltipX.value = e.clientX - rootRect.left + 10;
            tooltipY.value = e.clientY - rootRect.top - 28;
            return;
        }
        tooltipX.value = e.offsetX + 10;
        tooltipY.value = e.offsetY - 28;
    }
